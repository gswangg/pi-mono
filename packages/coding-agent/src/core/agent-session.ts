...
import { buildDiscoverableSlashCommandInfos, type SlashCommandInfo } from "./slash-commands.js";
...
export class AgentSession {
	...
	private _bindExtensionCore(runner: ExtensionRunner): void {
		const getCommands = (): SlashCommandInfo[] =>
			buildDiscoverableSlashCommandInfos({
				extensions: runner.getRegisteredCommands(),
	...
	private _isRetryableError(message: AssistantMessage): boolean {
		...
		// Match: overloaded_error, provider returned error, rate limit, 429, 500, 502, 503, 504, service unavailable, network/connection errors (including connection lost), fetch failed, request ended without sending chunks, HTTP/2 closed before response, terminated, retry delay exceeded, JSON parse errors (e.g. bad control characters in streaming)
		return /overloaded|provider.?returned.?error|rate.?limit|too many requests|429|500|502|503|504|service.?unavailable|server.?error|internal.?error|network.?error|connection.?error|connection.?refused|connection.?lost|other side closed|fetch failed|upstream.?connect|reset before headers|socket hang up|ended without|http2 request did not get a response|timed? out|timeout|terminated|retry delay|bad control character|json.*position/i.test(
			err,
		);
	}
